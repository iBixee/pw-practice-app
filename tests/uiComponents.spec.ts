...
test('checkboxes', async({page}) => {
    ...
    // const allBoxes = page.getByRole('checkbox')
    // for(const box of await allBoxes.all()){
    //     await box.uncheckcheck({force: true})
...
test('tooltips', async({page}) => { //type command + backslash to pause and enter debugg in inspect mode
    await page.getByText('Modal & Overlays').click()
...
test('dialog box', async({page}) => {
    ...
    page.on('dialog', dialog => { //when dealing with webpage dialog boxes you need to call on()
        expect(dialog.message()).toEqual("Are you sure you want to delete?")
        dialog.accept() //and then accept it
    })
...
// node.setAttribute('cx', '232.591')
// node.setAttribute('cy', '232.591')
// })
